...
app.get('/api/producao/ativos', async (req, res) => {
  try {
    ...
    const conditions = [
      "(pl.status IS NULL OR pl.status NOT IN ('pronto', 'entrega', 'entregue', 'finalizado', 'cancelado', 'rejeitado', 'em_rota'))",
      "(pl.setor_atual IS NULL OR pl.setor_atual NOT IN ('pronto', 'entrega', 'entregue', 'finalizado', 'cancelado', 'rejeitado', 'em_rota'))",
      "pl.setor_atual IN ('pcp_liberacao', 'pesagem', 'producao', 'moagem', 'laboratorio', 'laboratorio_revisao', 'laboratorio_amostras', 'coloracao', 'coloracao_revisao', 'coloracao_amostras', 'envase', 'envase_produzir', 'envase_enlatamento')"
    ];
    const params = [];
